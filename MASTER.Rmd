...
Now that we have our data how we want it, let's do some basic analysis. We will look at the dichotomous and continuous variables seperately.

#####Explore Dichotomous Variables

We want to determine the prevelance of each type of program or how many food providers offer each major type of program. These variables have simple dichotomous, yes/no, responses. We will use basic functions within R to calculate the statistics and a package called "pander" to format our tables.  

#####Meal Programs

Meal programs include the following services: hot meals, hom delivered meals, community kitchens, and meals for children.  
...
Grocery programs include the following services: food pantries, backpack programs, home delivered groceries, mobile pantries or markets, suppling food to other programs, or community gardens.  

The ditribution of grocery programs is:
...
This analysis tells us that grocery programs are more prevelant than meal programs. This trend is seen through 91.0%, or 222 organizations, offering grocery programs compared to just 30.3% offering meal programs.
...
Non-food related benefits include the following serivces: job training, houing assistance, utility assistance, legal assistance, GED or other education programs, healthcare, counseling, transportation services, clothing or furniture, referrals to other programs, Medicaid or CHIP, and financial assistance.  
...
This part of the analysis shows us the high prevlance of non-food realted benefits within the emergency food assistance network, suggesting the potentially larger role these organizations play in the social safety net. 76.2% of surveyed organizations provide some form of non-food related benefits, while only 23.4% did not provide any of these types of services, and 0.4% (or 1 pantry) did not know or refused to answer the question.
...
Now we will geocode the organization locations using the addresses provided. We will do this using code from https://qanda.science/r/3956223_batch-geocoding-with-googleway-r-how-to.jsp and the googleway package. You will need to get a Google API key before compelting this step. All directions can be found at the link provided.  

First, we have to make sure that our address variable is formated correctly. Then, we will geocode the addresses which will return the latitude and longitude coordiantes that will need to be formatted into their own variables. Geocoding takes a long time so after compeleting this step once I recommend writing the output to a csv file, with the STUDYID's, and then working from that csv file. We can then turn the code chunk off so it will not run when we knit the document by adding "eval=FALSE" to the beginning of the chunk.
...
Then we can set the intervals and color sheme for the color ramp for the poverty rates:
