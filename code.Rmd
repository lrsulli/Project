...
First we need to import the survey data. We will use the "readxl" package, so make sure this package is installed the first time you use it. We will import the three Excel files individually and place them in separate in data frames.
...
Now that we have each Excel file in a data frame we need to merge them into one data frame for our analysis. To do this we need to have a common variable in each data frame, in this case a "Study ID" was given to each provider when they answered the surveyed. We will merge using this identifyer. We will also prepare each data frame for merging by renaming variables where necessary and only keeping a subset of the variables to keep things simplier.  
...
We don't need all of these variables for our analysis. To make things simpler, we will only keep what we need. We will also make a copy of the original data frame just in case we miss something and want to come back and get it later.
