...
```{r Merge Data: Census}
data_census = merge(data_census2, data_census3, by="tr10fips")

data_census = data.frame(data_census)
```

```{r Merge Data: Census with Data}
data = merge(data, data_census1, by="STUDYID")
data = merge(data, data_census, by='tr10fips')
...
# Drop state ID from census tract number
...
```{r}
# Keep Wayne, Macomb, and Oakland Counties (FIPS = 163, 099, and 125)
map_counties = subset(tractMap, CNTY_CODE=='163' | CNTY_CODE=='099' | CNTY_CODE=='125')
data_counties = subset(data_census, county_fips=='163' | county_fips=='099' | county_fips=='125')
data_counties = data_counties[!duplicated(data_counties[c('tract_short')]),]

# Merge with census data
map_back = merge(map_counties,data_counties,by.x='LINK', by.y='tract_short', all.x=F)
```

```{r}
# Drop multiple census tracts
unique_tracts = data[!duplicated(data[c('tract_short')]),]
```

```{r}
map_merge = merge(tractMap,unique_tracts,by.x='LINK', by.y='tract_short', all.x=F)
```

```{r}
plot(map_back, col='black')
```

### Plot Map by poverty rate

```{r}
varToPLot=map_final$pov_rateT
```
...
plot(map_back, col='grey')
plot(map_final, col = colorPallette, main=title,border='black',add=T)
